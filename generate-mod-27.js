...
function generate(moduleName, dirPath, items) {
  ...
  items.forEach(item => {
    // Sanitize item specifically handling slashes and spaces
...
`;
  ...
  console.log(`Generated ${items.length} premium audit components for ${moduleName}`);
}
...
generate("audit", "27-audit", auditItems);
