...
export default function TeacherReviewSubmissionModal() {
  const { isReviewModalOpen, closeReviewModal, selectedSubmissionForReview, selectedHomework } = useTeacherAssignmentsStore();
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 flex flex-col h-full max-h-[80vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="px-6 py-4 border-t border-border bg-card flex justify-between items-center shrink-0 -mx-6 -mb-6 mt-6">
            ...
            <div className="flex gap-3">
              ...
              <button type="button" disabled={!!successAction} onClick={() => handleAction('Approved & Graded')} className="px-4 py-2 bg-success text-white font-bold text-[13px] rounded-lg hover:bg-success/90 flex items-center gap-2 transition-colors disabled:opacity-50">
