...
import { X, FileBadge, CheckCircle, XCircle, FileText } from 'lucide-react';
import { usePrincipalDocumentsStore } from '../documents_store/usePrincipalDocumentsStore';
import { updateCertRequestStatus } from '../documents_api/PrincipalDocumentsApi';
import clsx from 'clsx';

export default function PrincipalCertificateApprovalModal() {
  const { selectedRequest, setSelectedRequest } = usePrincipalDocumentsStore();
  ...
  const handleAction = async (action: 'Approve' | 'Reject') => {
    setLoading(true);
    await updateCertRequestStatus(selectedRequest.id, action, remarks);
      ...
      <div className="w-full max-w-xl bg-bg-main border border-border rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
        ...
        <div className="px-5 py-4 bg-card border-b border-border flex items-center justify-between shrink-0">
          <h2 className="text-[16px] font-bold text-text-primary flex items-center gap-2">
            <FileBadge className="text-primary" size={18} /> 
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-5">
          ...
          <div className="flex items-start justify-between gap-4 border-b border-border pb-4">
            <div>
              <h3 className="text-[20px] font-bold text-text-primary mb-1">{selectedRequest.type}</h3>
            ...
            <span className={clsx("inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border", 
              selectedRequest.urgency === 'Urgent' ? 'bg-danger/10 text-danger border-danger/30' : 'bg-success/10 text-success border-success/30'
            )}>
          ...
          <div className="bg-card border border-border p-4 rounded-lg">
            ...
            <p className="text-[14px] text-text-primary">{selectedRequest.reason}</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea 
              ...
              placeholder="Add any remarks or conditions here..."
            />
          </div>
