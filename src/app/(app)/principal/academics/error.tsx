...
export default function PrincipalAcademicsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Principal Academics Error:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-[400px] flex items-center justify-center p-6">
      <div className="bg-card border border-border rounded-xl p-8 max-w-md w-full text-center shadow-2xl shadow-black/50">
        ...
        <h2 className="text-[20px] font-bold text-text-primary mb-2">Failed to load Academic Data!</h2>
        <p className="text-[14px] text-text-secondary mb-8">
          We encountered an issue fetching the academic structure and syllabus progress. Please try again.
        </p>
        <button
          onClick={() => reset()}
