...
export default function PrincipalFeesError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Principal Fees Error:", error);
  }, [error]);
  ...
  return (
    <div className="min-h-[400px] flex items-center justify-center p-6">
      <div className="bg-card border border-border rounded-xl p-8 max-w-md w-full text-center shadow-2xl shadow-black/50">
        ...
        <h2 className="text-[20px] font-bold text-text-primary mb-2">Failed to load Fee Data!</h2>
        <p className="text-[14px] text-text-secondary mb-8">
          We encountered an issue fetching the financial overview or approval requests. Please try again.
        </p>
        <button
          onClick={() => reset()}
