"use client";
import React, { useEffect, useState } from 'react';
import { PrincipalHealthStudent } from '../health_types/PrincipalHealth.types';
import { fetchPrincipalHealthStudents } from '../health_api/PrincipalHealthApi';
import { Search, Filter, AlertTriangle, Droplets } from 'lucide-react';
import { usePrincipalHealthStore } from '../health_store/usePrincipalHealthStore';
import clsx from 'clsx';

export default function PrincipalHealthRecordsTab() {
  const [students, setStudents] = useState<PrincipalHealthStudent[]>([]);
  const [loading, setLoading] = useState(true);
  const { setSelectedStudent } = usePrincipalHealthStore();

  useEffect(() => {
    let isMounted = true;
    fetchPrincipalHealthStudents().then(data => {
      if (isMounted) {
        setStudents(data);
        setLoading(false);
      }
    });
    return () => { isMounted = false; };
  }, []);

  if (loading) {
    return (
      <div className="bg-card border border-border rounded-lg p-6">
        <div className="space-y-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-20 bg-skeleton-base animate-pulse rounded" />)}
        </div>
      </div>
    );
  }

  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
      <div className="p-5 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-[18px] font-bold text-text-primary">Health Records</h2>
          <p className="text-[13px] text-text-secondary">View student health profiles, medical conditions, and allergies.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="flex items-center gap-2 bg-input border border-border rounded-md px-3 py-1.5 focus-within:border-primary transition-colors">
            <Search size={14} className="text-text-secondary" />
            <input 
              type="text" 
              placeholder="Search student, blood group..." 
              className="bg-transparent border-none outline-none text-[13px] text-text-primary w-full sm:w-64 placeholder:text-text-secondary/50"
            />
          </div>
          <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.dispatchEvent(new CustomEvent('open-coming-soon', { detail: 'Action completed successfully!' })); }} className="flex items-center justify-center gap-2 px-4 py-1.5 bg-page hover:bg-white/5 border border-border rounded-md text-[13px] font-bold text-text-primary transition-colors">
            <Filter size={14} /> Filter
          </button>
        </div>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-primary/5 border-b border-border text-[12px] font-semibold text-text-secondary uppercase tracking-wider">
              <th className="p-4 w-64">Student Info</th>
              <th className="p-4 w-32">Blood Group</th>
              <th className="p-4 w-64">Conditions / Allergies</th>
              <th className="p-4 w-40 text-center">Alert Level</th>
              <th className="p-4 w-32 text-right">Action</th>
            </tr>
          </thead>
          <tbody>
            {students.map((student) => (
              <tr key={student.id} className="border-b border-border/50 hover:bg-white/5 transition-colors group">
                <td className="p-4">
                  <p className="text-[14px] font-bold text-text-primary mb-1">{student.studentName}</p>
                  <p className="text-[12px] text-text-secondary">Class: {student.classAndSection} | ID: {student.studentId}</p>
                </td>
                <td className="p-4">
                  <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[12px] font-bold bg-danger/10 text-danger border border-danger/20">
                    <Droplets size={12}/> {student.bloodGroup}
                  </span>
                </td>
                <td className="p-4">
                  {student.medicalConditions.length > 0 ? (
                    <p className="text-[12px] font-bold text-warning mb-1">{student.medicalConditions.join(', ')}</p>
                  ) : <p className="text-[12px] text-text-secondary mb-1">No major conditions</p>}
                  
                  {student.allergies.length > 0 && (
                     <p className="text-[11px] text-danger">Allergic to: {student.allergies.join(', ')}</p>
                  )}
                </td>
                <td className="p-4 text-center">
                  {student.alertLevel ? (
                    <span className={clsx("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border", 
                      student.alertLevel === 'High' ? 'bg-danger/10 text-danger border-danger/30' :
                      student.alertLevel === 'Medium' ? 'bg-warning/10 text-warning border-warning/30' :
                      'bg-info/10 text-info border-info/30'
                    )}>
                      {student.alertLevel === 'High' && <AlertTriangle size={12}/>}
                      {student.alertLevel} Alert
                    </span>
                  ) : (
                    <span className="text-[12px] text-text-secondary">-</span>
                  )}
                </td>
                <td className="p-4 text-right">
                  <button 
                    onClick={() => setSelectedStudent(student)}
                    className="px-4 py-1.5 rounded bg-primary/10 hover:bg-primary border border-primary/30 hover:border-primary text-[12px] font-bold text-primary hover:text-black transition-colors"
                  >
                    View Profile
                  </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
