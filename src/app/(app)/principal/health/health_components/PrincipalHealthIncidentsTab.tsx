...
import { PrincipalMedicalIncident } from '../health_types/PrincipalHealth.types';
import { fetchPrincipalMedicalIncidents } from '../health_api/PrincipalHealthApi';
...
export default function PrincipalHealthIncidentsTab() {
  const [incidents, setIncidents] = useState<PrincipalMedicalIncident[]>([]);
  const [loading, setLoading] = useState(true);
  const { setSelectedIncident } = usePrincipalHealthStore();

  useEffect(() => {
    let isMounted = true;
    fetchPrincipalMedicalIncidents().then(data => {
      if (isMounted) {
        setIncidents(data);
        setLoading(false);
      }
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
      <div className="p-5 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-[18px] font-bold text-text-primary">Medical Incidents & Emergencies</h2>
          <p className="text-[13px] text-text-secondary">Monitor medical incidents, actions taken, and emergency cases.</p>
        </div>
              ...
              <tr key={incident.id} className="border-b border-border/50 hover:bg-white/5 transition-colors group">
                <td className="p-4">
                  <span className={clsx("text-[11px] font-bold px-2 py-0.5 rounded mb-2 inline-block border", 
                    incident.severity === 'Emergency' ? 'bg-danger/10 text-danger border-danger/30' :
                    incident.severity === 'Major' ? 'bg-warning/10 text-warning border-warning/30' :
                    'bg-info/10 text-info border-info/30'
                  )}>
                    {incident.severity}
                  </span>
                  <p className="text-[14px] font-bold text-text-primary mb-1">{incident.incidentType}</p>
                  <p className="text-[12px] text-text-secondary">{incident.date} | {incident.time}</p>
                </td>
                <td className="p-4">
                  <p className="text-[14px] font-bold text-text-primary mb-1">{incident.studentName}</p>
                  <p className="text-[12px] text-text-secondary">Class: {incident.classAndSection} | ID: {incident.studentId}</p>
                </td>
                <td className="p-4">
                  <p className="text-[13px] text-text-primary line-clamp-2 mb-1">{incident.description}</p>
                  <p className="text-[11px] text-text-secondary"><span className="font-bold text-success">Action:</span> {incident.actionTaken}</p>
                </td>
                <td className="p-4 text-center">
                  <span className={clsx("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border", 
                    incident.status === 'Open' ? 'bg-warning/10 text-warning border-warning/30' : 'bg-success/10 text-success border-success/30'
                  )}>
                    {incident.status === 'Open' && <Activity size={12}/>}
                    {incident.status}
                  </span>
                </td>
