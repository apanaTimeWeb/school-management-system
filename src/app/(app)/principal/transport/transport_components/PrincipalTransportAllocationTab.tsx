...
import { PrincipalTransportStudent } from '../transport_types/PrincipalTransport.types';
import { fetchPrincipalTransportStudents } from '../transport_api/PrincipalTransportApi';
import { Search, Filter, MapPin, CheckCircle, XCircle } from 'lucide-react';
import clsx from 'clsx';

export default function PrincipalTransportAllocationTab() {
  const [students, setStudents] = useState<PrincipalTransportStudent[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let isMounted = true;
    fetchPrincipalTransportStudents().then(data => {
      if (isMounted) {
        setStudents(data);
        setLoading(false);
      }
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
      <div className="p-5 border-b border-border flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-text-secondary">View students allocated to routes and their daily bus attendance.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="flex items-center gap-2 bg-input border border-border rounded-md px-3 py-1.5 focus-within:border-primary transition-colors">
            ...
            <input 
              type="text" 
              placeholder="Search student, route..." 
        ...
        <table className="w-full text-left border-collapse min-w-[1000px]">
          <thead>
            <tr className="bg-primary/5 border-b border-border text-[12px] font-semibold text-text-secondary uppercase tracking-wider">
              ...
              <th className="p-4 w-64">Pickup/Drop Stop</th>
              <th className="p-4 w-40 text-center">Today's Attendance</th>
            </tr>
          </thead>
          <tbody>
            {students.map((student) => (
              <tr key={student.id} className="border-b border-border/50 hover:bg-white/5 transition-colors group">
                ...
                <td className="p-4">
                  <p className="text-[14px] font-bold text-text-primary mb-1">{student.routeName}</p>
                  <p className="text-[12px] font-mono text-text-secondary bg-black/20 inline-block px-1.5 rounded">{student.routeId}</p>
                </td>
                <td className="p-4">
                  <p className="text-[13px] font-bold text-text-primary flex items-center gap-2"><MapPin size={14} className="text-warning"/> {student.pickupStop}</p>
                </td>
                <td className="p-4 text-center">
                  <span className={clsx("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold border", 
                    ...
                    {student.todayAttendance === 'Present' ? <CheckCircle size={12}/> : <XCircle size={12}/>}
