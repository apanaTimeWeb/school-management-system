          ...
          <div className="space-y-2">
            {leaves.length === 0 ? <p className="text-text-secondary text-[13px] text-center p-4">No leave requests.</p> : leaves.map(l => (
              <div key={l.id} className="p-3 bg-page border border-border/50 rounded flex justify-between items-center hover:-translate-y-0.5 transition-all duration-200">
                ...
                <button className="px-3 py-1 bg-primary text-black text-[12px] font-medium rounded hover:bg-primary-hover transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-page">Review</button>
              </div>
            ))}
          </div>
        )}
