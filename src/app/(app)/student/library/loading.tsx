import React from 'react';

export default function StudentLibraryLoading() {
  return (
    <div className="w-full max-w-[1400px] mx-auto p-4 md:p-6 lg:p-8 motion-safe:animate-pulse">
      {/* Header Skeleton */}
      <div className="mb-6">
        <div className="w-48 h-8 bg-skeleton-base rounded-md mb-2"></div>
        <div className="w-72 h-4 bg-skeleton-base rounded-md"></div>
      </div>

      <div className="flex flex-col gap-6">
        {/* Tabs Skeleton */}
        <div className="w-96 h-12 bg-card border border-border rounded-lg bg-skeleton-base"></div>

        {/* Search Skeleton */}
        <div className="w-full max-w-xl h-12 bg-card border border-border rounded-full bg-skeleton-base"></div>

        {/* Content Grid Skeleton */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
             <div key={i} className="bg-card border border-border rounded-xl h-[300px] bg-skeleton-base"></div>
          ))}
        </div>
      </div>
    </div>
  );
}
