...
import type { MedicalNotice } from '../student_health_types/student_health_types';
import { HeartHandshake, AlertTriangle, Syringe } from 'lucide-react';
...
export default function StudentHealthNotices({ notices }: Props) {
  ...
  if (notices.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-10 text-center text-text-secondary bg-page border border-border rounded-xl">
        <HeartHandshake size={48} className="text-border mb-4" />
        <h3 className="text-lg font-bold text-text-primary">No Notices</h3>
        <p className="text-sm">There are currently no health advisories or notices.</p>
      </div>
    );
  }
      ...
      {notices.map(notice => (
        <div key={notice.id} className={clsx(
          ...
          <div className={clsx(
            ...
            {notice.isUrgent ? <AlertTriangle size={20} /> : <Syringe size={20} />}
          </div>
          <div className="flex-1">
            ...
            <p className="text-sm text-text-secondary leading-relaxed">{notice.message}</p>
          </div>
        </div>
      ))}
    </div>
