...
export default function StudentLeaveError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Student Leave Error:", error);
  }, [error]);

  return (
    <div className="w-full h-[60vh] flex flex-col items-center justify-center p-6 text-center">
      <div className="w-16 h-16 rounded-full bg-danger/10 flex items-center justify-center mb-4">
        <FileX size={32} className="text-danger" />
      </div>
      <h2 className="text-xl font-bold text-text-primary mb-2">Could not load leave records</h2>
      <p className="text-sm text-text-secondary max-w-md mb-6">
        There was an error retrieving your leave history. Please try again.
      </p>
      <button
        onClick={() => reset()}
