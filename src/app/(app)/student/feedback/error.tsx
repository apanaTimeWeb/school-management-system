...
export default function StudentFeedbackError({
  error,
  reset,
}: {
  ...
  return (
    <div className="w-full h-[60vh] flex flex-col items-center justify-center p-6 text-center">
      <div className="w-16 h-16 rounded-full bg-danger/10 flex items-center justify-center mb-4">
        <MessageSquareOff size={32} className="text-danger" />
      </div>
      <h2 className="text-xl font-bold text-text-primary mb-2">Could not load feedback portal</h2>
      <p className="text-sm text-text-secondary max-w-md mb-6">
        There was an error connecting to the feedback server. Please check your connection and try again.
      </p>
      <button
        onClick={() => reset()}
