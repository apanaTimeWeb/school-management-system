...
export default function ClassroomAudit() {
  const [filter, setFilter] = useState('All');

  const logs = [
    { type: 'Marks', who: 'Mr. Ramesh (TCH-04)', role: 'Teacher', what: 'Modified Term 1 Marks (STU-102)', old: 'Science: 45/100', new: 'Science: 55/100', when: '12-Oct-2025, 08:30 AM', ip: '117.22.45.19 (Android/Chrome)' },
    { type: 'Attendance', who: 'Mrs. Anita (TCH-11)', role: 'Class Teacher', what: 'Changed Daily Attendance (STU-089)', old: 'Status: Absent (A)', new: 'Status: Present (P)', when: '11-Oct-2025, 01:15 PM', ip: '192.168.1.55 (Win/Edge)' },
    { type: 'Marks', who: 'Admin (admin@erp)', role: 'Super Admin', what: 'Unlocked Marksheet (Class 10-A)', old: 'Status: Locked', new: 'Status: Editable', when: '10-Oct-2025, 10:00 AM', ip: '192.168.1.45 (Win/Chrome)' },
  ];
    ...
    <div className="flex flex-col gap-6 h-full fade-in relative">
      <div className="flex-1 bg-card border border-border rounded-xl shadow-sm p-6 overflow-y-auto">
        <h2 className="text-xl font-bold text-text-primary border-b border-border pb-4 mb-6 flex items-center gap-2">
          <ClipboardList size={20} className="text-warning"/> Classroom (Marks & Attendance) Audit
        </h2>
        
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
           <p className="text-sm font-semibold text-text-secondary max-w-2xl">
             Highly sensitive ledger tracking post-submission modifications to Attendance registers and Exam marksheets.
           </p>
           ...
           <div className="flex items-center gap-2 bg-bg-page border border-border rounded-lg p-1">
             ...
             <select value={filter} onChange={(e)=>setFilter(e.target.value)} className="bg-transparent border-none text-sm font-bold outline-none px-2 py-1 cursor-pointer">
               <option value="All">All Classroom Logs</option>
                   ...
                   <tr key={i} className="hover:bg-bg-page transition">
                     <td className="p-3 border-b border-border">
                       <div className="flex flex-col">
                         <span className="font-bold text-warning">{log.who}</span>
