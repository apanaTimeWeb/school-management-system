                  ...
                  <td className="p-4 text-right">
                    {app.status === 'Pending' ? (
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => approve(app.id)} className="flex items-center gap-1 px-3 py-1.5 bg-success/10 text-success border border-success/20 rounded-md font-bold text-xs hover:bg-success hover:text-white transition-all active:scale-95 shadow-sm">
                          <Check size={14} /> Approve
                        </button>
                        <button onClick={() => reject(app.id)} className="flex items-center gap-1 px-3 py-1.5 bg-danger/10 text-danger border border-danger/20 rounded-md font-bold text-xs hover:bg-danger hover:text-white transition-all active:scale-95 shadow-sm">
                          <X size={14} /> Reject
                        </button>
                      </div>
                    ) : (
