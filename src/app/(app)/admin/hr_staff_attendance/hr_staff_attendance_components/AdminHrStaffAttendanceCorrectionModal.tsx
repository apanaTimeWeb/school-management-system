...
interface AdminHrStaffAttendanceCorrectionModalProps {
  isOpen: boolean;
  close: () => void;
  context: { employeeId: string, name: string, day: number } | null;
}

export default function AdminHrStaffAttendanceCorrectionModal({ isOpen, close, context }: AdminHrStaffAttendanceCorrectionModalProps) {
  const [newStatus, setNewStatus] = useState("Present");

  if (!isOpen || !context) return null;

  const handleSave = () => {
    alert(`Mock Save: Attendance for ${context.name} on Day ${context.day} corrected to ${newStatus}.`);
    close();
  };
      ...
      <div className="bg-overlay border border-border rounded-xl w-full max-w-md shadow-2xl overflow-hidden motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-95">
        ...
        <div className="p-6">
          ...
          <div className="mb-6">
             ...
             <textarea className="w-full bg-input border border-border rounded-md px-3 py-2 text-sm text-foreground focus:border-primary outline-none min-h-[80px]" placeholder="E.g. Forgot to punch in, Biometric machine error..."></textarea>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            ...
            <button 
              onClick={handleSave} 
              className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-white rounded-md shadow-lg transition-all active:scale-95 bg-warning hover:bg-yellow-500 shadow-warning/20 text-black"
            >
