# Hr / Office Dashboard — Feature Map
...
|---|---|---|
| `hr_dashboard_components/` | Renders all visual KPI widgets, pending lists, and alert banners | `HrDashboardMain.tsx`, `HrDashboardKpis.tsx`, `HrDashboardAttendance.tsx`, `HrDashboardAlerts.tsx` |
| `hr_dashboard_api/` | Simulated API fetches for dashboard aggregation | `HrDashboardApi.ts` |
| `hr_dashboard_types/` | TypeScript interfaces for dashboard data and alerts | `HrDashboardTypes.ts` |
| `hr_dashboard_constants/` | Hardcoded configuration and fallback mock data | `HrDashboardConstants.ts` |

## Feature Inventory
| Feature | Route | What the User Can Do | Key Components | Main API Calls | Status |
|---|---|---|---|---|---|
| HR Core Stats | `/admin/hr_dashboard` | View total employee split (Teachers vs Non-Teaching) | `HrDashboardKpis` | `GET /api/hr/dashboard/stats` | ✅ MOCKED |
| Attendance KPIs | `/admin/hr_dashboard` | Monitor today's attendance percentage and absentees | `HrDashboardAttendance` | `GET /api/hr/dashboard/stats` | ✅ MOCKED |
| Pending Items | `/admin/hr_dashboard` | View lists of expiring documents and leave requests | `HrDashboardPendingItems` | `GET /api/hr/dashboard/stats` | ✅ MOCKED |
| Staff Events | `/admin/hr_dashboard` | See upcoming birthdays and work anniversaries | `HrDashboardEvents` | `GET /api/hr/dashboard/stats` | ✅ MOCKED |

## User Flows & Interactions
### Flow 1: View Dashboard
1. User navigates to `/admin/hr_dashboard`
2. Next.js App Router loads `page.tsx` and delegates to `HrDashboardMain` client component
...
|---|---|---|---|---|
| `fetchHrDashboardStats()` | GET | `/api/hr/dashboard/stats` | — | `HrDashboardStats` |
...
- **Component File Ceiling:** We split the dashboard into 5+ sub-components precisely to avoid the `HrDashboardMain.tsx` breaching the 300-line ceiling. Do not merge them back.
- **No Arbitrary Colors:** Status indicators in `HrDashboardAlerts` use strict mappings to CSS tokens (e.g., `--warning`, `--danger`). Do not use raw hex values.

## Component Responsibility Map
| Component File | Responsibility |
|---|---|
| `HrDashboardMain.tsx` | Root client orchestrator. Fetches data, manages loading/error states, renders children. |
| `HrDashboardKpis.tsx` | Renders the top row of core employee statistics. |
| `HrDashboardAttendance.tsx` | Renders attendance and leave-related statistics. |
| `HrDashboardPendingItems.tsx` | Displays lists of actionable items (docs, leaves) needing attention. |
| `HrDashboardEvents.tsx` | Displays upcoming birthdays and anniversaries. |
| `HrDashboardAlerts.tsx` | Top-level alert banner for high-priority warnings. |
