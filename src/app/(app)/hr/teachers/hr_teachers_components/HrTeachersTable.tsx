...
import { Eye, Edit, ShieldAlert, CheckCircle } from "lucide-react";
import { useRouter } from "next/navigation";
import type { Teacher } from "../hr_teachers_types/HrTeachersTypes";
import { HrTeachersUrlConfig } from "../hr_teachers_url_config";
import HrTeacherEditModal from "./HrTeacherEditModal";
import HrTeacherStatusModal from "./HrTeacherStatusModal";

interface HrTeachersTableProps {
  teachers: Teacher[];
}

export default function HrTeachersTable({ teachers: initialTeachers }: HrTeachersTableProps) {
  ...
  const handleSaveEdit = (updatedTeacher: Teacher) => {
    setTeachersList(prev => prev.map(t => t.id === updatedTeacher.id ? updatedTeacher : t));
    setEditTeacher(null);
  };
  ...
  return (
    <>
              ...
              <tr key={t.id} className="border-b border-border hover:bg-primary/5 transition-colors group">
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button 
                      onClick={() => router.push(HrTeachersUrlConfig.routes.profile(t.id))}
                    ...
                    <button 
                      onClick={() => setStatusTeacher(t)}
                      className={`p-1.5 ${t.status === 'Active' ? 'text-danger bg-danger/10 hover:bg-danger' : 'text-success bg-success/10 hover:bg-success'} hover:text-white rounded-md transition-all active:scale-95 shadow-sm`} 
                      title={t.status === 'Active' ? 'Suspend Teacher' : 'Activate Teacher'}
                    >
                      {t.status === 'Active' ? <ShieldAlert size={16} /> : <CheckCircle size={16} />}
                    </button>
                  </div>
                </td>
              </tr>
      ...
      <HrTeacherStatusModal
        isOpen={!!statusTeacher}
        teacher={statusTeacher}
        onClose={() => setStatusTeacher(null)}
