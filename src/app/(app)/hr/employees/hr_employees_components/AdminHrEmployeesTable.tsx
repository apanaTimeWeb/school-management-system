...
import AdminHrEmployeeEditModal from "./AdminHrEmployeeEditModal";
import AdminHrEmployeeDeleteModal from "./AdminHrEmployeeDeleteModal";

interface AdminHrEmployeesTableProps {
  employees: Employee[];
}

export default function AdminHrEmployeesTable({ employees: initialEmployees }: AdminHrEmployeesTableProps) {
              ...
              <tr key={emp.id} className="border-b border-border hover:bg-input/30 transition-colors group">
                ...
                <td className="p-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button 
                      onClick={() => router.push(AdminHrEmployeesUrlConfig.routes.profile(emp.id))}
