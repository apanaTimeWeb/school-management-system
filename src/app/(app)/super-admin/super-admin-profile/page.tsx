import { User } from "lucide-react";
import SuperAdminSuperAdminProfileConfig from "./super_admin_super_admin_profile_components/SuperAdminSuperAdminProfileConfig";

export default function SuperAdminSuperAdminProfilePage() {
  return (
    <div className="flex flex-col gap-6 max-w-[1000px] mx-auto pb-12">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold text-text-primary flex items-center gap-2">
            53. 👑 Super Admin Profile
          </h1>
        </div>
      </div>

      <SuperAdminSuperAdminProfileConfig />
    </div>
  );
}
