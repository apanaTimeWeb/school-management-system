...
import { Shield, School, GraduationCap, ArrowLeft, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-bg-main flex flex-col items-center justify-center p-6 font-sans relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
        ...
        {/* Principal Card */}
        <Link href="/login/principal" className="group bg-white border border-border rounded-2xl p-8 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col items-center text-center">
          <div className="w-20 h-20 rounded-full bg-yellow-500/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
            <GraduationCap className="text-yellow-600" size={40} />
          </div>
          <h2 className="text-2xl font-bold text-text-primary mb-2">PRINCIPAL</h2>
          <p className="text-text-secondary font-medium mb-8">Executive Overview & Dashboards</p>
          <div className="mt-auto flex items-center gap-2 text-yellow-600 font-bold bg-yellow-500/5 px-6 py-3 rounded-full group-hover:bg-yellow-500 group-hover:text-white transition-colors w-full justify-center">
            Continue <ArrowRight size={18} />
          </div>
        </Link>
