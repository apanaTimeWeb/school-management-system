...
export default function LibrarianSidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`fixed top-0 left-0 h-screen bg-[#3B0764] text-white/90 w-[280px] z-50 border-r border-white/10 shadow-xl transition-transform duration-300 ease-in-out flex flex-col ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        
        {/* Header */}
        <div className="h-16 flex items-center justify-between px-6 bg-[#3B0764] border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-[#A855F7] rounded-lg flex items-center justify-center text-white shadow-md">
              ...
              <Link 
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-200 group ${
                  isActive 
                    ...
                    : 'text-white/60 hover:bg-[#6D28D9]/50 hover:text-white'
                }`}
              >
                <item.icon className={`w-5 h-5 transition-colors ${isActive ? 'text-white' : 'text-white/60 group-hover:text-white'}`} />
