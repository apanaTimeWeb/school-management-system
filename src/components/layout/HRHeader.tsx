    ...
    <header className="h-16 bg-header/90 backdrop-blur-md border-b border-border sticky top-0 z-30 flex items-center justify-between px-4 lg:px-8 transition-all duration-300">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="lg:hidden p-2 -ml-2 rounded-md text-sidebar-text-muted hover:bg-[#BE185D]/10 hover:text-[#BE185D] transition-colors"
        >
          <Menu size={24} className="text-[#BE185D]" />
        </button>
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            className={clsx(
              ...
              showNotifications ? "bg-indigo-100 text-[#BE185D] shadow-sm" : "bg-transparent hover:bg-[#BE185D]/10 text-sidebar-text-muted hover:text-[#BE185D]"
            )}
          >
            <Bell size={20} className="text-[#EC4899]" />
            {unreadCount > 0 && (
              <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 text-[#EC4899] rounded-full border-2 border-header flex items-center justify-center"></span>
            )}
          </button>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 bg-card border border-border rounded-xl shadow-xl overflow-hidden fade-in z-50">
              <div className="flex justify-between items-center p-3 border-b border-border bg-bg-page">
                <h3 className="font-bold text-sm text-text-[#BE185D]">Notifications</h3>
                  ...
                  <div key={notif.id} className={clsx("p-3 border-b border-border hover:bg-bg-page transition cursor-pointer flex gap-3", !notif.read && "bg-[#BE185D]/10/50")}>
                    ...
                    <div className="flex flex-col flex-1">
                      <div className="flex justify-between items-start gap-2">
                         <span className={clsx("text-sm font-bold text-text-[#BE185D]", !notif.read && "text-[#BE185D]")}>{notif.title}</span>
        ...
        <div ref={profileRef} className="relative">
          <button
            ...
            className={clsx(
              ...
              showProfileMenu ? "bg-[#BE185D]/10 shadow-sm" : "bg-transparent hover:bg-[#BE185D]/10"
            )}
          >
            <UserCircle size={28} className={clsx("transition-colors", showProfileMenu ? "text-[#BE185D]" : "text-indigo-400 group-hover:text-[#BE185D]")} />
            <div className="hidden sm:flex flex-col items-start leading-tight">
              <span className={clsx("text-sm font-semibold transition-colors", showProfileMenu ? "text-[#BE185D]" : "text-sidebar-text group-hover:text-[#BE185D]")}>Neha K.</span>
              <span className="text-xs text-sidebar-text-muted hidden md:block">HR Manager</span>
            </div>
            <ChevronDown size={16} className={clsx("ml-1 hidden sm:block transition-colors", showProfileMenu ? "text-[#BE185D]" : "text-sidebar-text-muted group-hover:text-[#BE185D]")} />
          </button>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-lg shadow-xl overflow-hidden fade-in z-50 py-1">
              <Link href="/hr/profile" className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-text-[#BE185D] hover:bg-bg-page hover:text-[#BE185D] transition border-b border-border">
                <UserCircle size={16} className="text-indigo-500" /> My Profile
              </Link>
              <Link href="/login" className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold text-danger hover:text-[#EC4899]/10 transition">
