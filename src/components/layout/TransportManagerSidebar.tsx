...
const MENU_ITEMS = [
  ...
  { href: '/transport-manager/notifications', label: 'Notifications', icon: <Bell size={20} className="text-white" /> },
...
export default function TransportManagerSidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 z-50 h-screen w-[280px] bg-[#164E63] border-r border-white/10 transition-transform duration-300 ease-in-out
        ${isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Logo Area */}
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/10 bg-[#164E63] shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#06B6D4] flex items-center justify-center text-white font-extrabold text-[18px]">
              T
            </div>
            <span className="font-bold text-[18px] text-white tracking-tight">Transport<span className="text-[#06B6D4]">Pro</span></span>
          </div>
          <button 
        ...
        <nav className="p-4 space-y-1.5 overflow-y-auto h-[calc(100vh-140px)] custom-scrollbar">
          <p className="text-[11px] font-bold text-white/60 uppercase tracking-wider mb-3 px-2">Transport Modules</p>
              ...
              <Link 
                ...
                className={`
                  ...
                  ${isActive 
                    ...
                    : 'text-white/60 hover:text-white hover:bg-[#0891B2] border border-transparent'}
                `}
              >
          ...
          <Link 
            ...
            `}
          >
            <span className={pathname.startsWith('/transport-manager/my-profile') ? 'text-white' : 'text-white/60'}>
              <User size={20} />
            </span>
            <span>25 - My Profile</span>
          </Link>
          
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-white/60 hover:text-white hover:bg-[#0891B2] transition-colors font-semibold text-[13px]">
